...
class my_ocr_view extends StatelessWidget {
  const my_ocr_view({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyOCRPage(title: 'Flutter Demo Home Page'),
    );
  }
}

class MyOCRPage extends StatefulWidget {
  const MyOCRPage({Key? key, required this.title}) : super(key: key);

  final String title;
  @override
  State<MyOCRPage> createState() => _MyOCRPagePageState();
}

class _MyOCRPagePageState extends State<MyOCRPage> {
  ...
  Future _onCallingAPI() async {
    if (key != null && image != null) {
      ...
      final response = await http.post(
        Uri.parse('${SettingFile().host}${SettingFile().pathIDOCR}')
